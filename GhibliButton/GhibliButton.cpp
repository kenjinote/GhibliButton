...
bool CopyImage(const LPPICTURE pPic)
{
    bool result = false;
    if (OpenClipboard(NULL))
    {
        EmptyClipboard();
        HBITMAP hbitmap;
        pPic->get_Handle((OLE_HANDLE FAR*) & hbitmap);
        DIBSECTION ds;
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    static LPCWSTR lpszButtonName[] = { L"すばらしい！", L"ほう..", L"またねっ", L"ＯＫ", L"ハハハハッ", L"いいね！", L"ありがとう①", L"フフフ", L"なるほど", L"ほほぅ", L"ありがとう②", L"ゴォォォ・・・",L"やなヤツやなヤツ",L"♩♪♩♩♫♪",L"！",L"いいよ",L"おっと",L"ごめんね",L"またね",L"書いてね" };
    ...
    case WM_LBUTTONDOWN:
        if (lp_Picture) {
            if (CopyImage(lp_Picture)) {
                MessageBox(hWnd, L"クリップボードに画像をコピーしました。", L"確認", 0);
            }
        }
        break;
    case WM_DESTROY:
